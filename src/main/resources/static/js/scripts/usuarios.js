    ...
    ajaxRequest("PUT", `${url}/${id}/deactivate`)
        .done((data) => {
            if (data.ok) {
                Swal.fire({
                    icon: 'success',
                    title: 'Se ha desactivado el usuario',
                    showConfirmButton: false,
                    timer: 1500
                });
                getTabla();
        ...
        .done((data) => {
            ...
            if (data.ok) {
                $.each(data.body, (index, registro) => {
                    const roles = registro.roles.map((rol) => rol.nombre).join(", ");
                    let botonera = `
                        ...
                        if(registro.activo == true){
                            botonera += `
                            <button type="button" class="btn btn-danger btn-xs desactivar">
                                <i class="fas fa-times"></i>`;
                        }
                        else{
                            botonera += `
                            <button type="button" class="btn btn-info btn-xs activar">
                                <i class="fas fa-check"></i>`;
                        }     
                    t.row.add([botonera, registro.id, registro.email,roles, registro.activo ? "Activo" : "Inactivo"]);
                });
                t.draw(false);
            } else {
